 ...
 * This script fetches the latest block from the Ethereum network
...
const rpcUrl = process.env.ETHRPCURL;
const provider = new ethers.JsonRpcProvider(rpcUrl);

// Log file path
const logFilePath = path.join(logsDir, 'eth-transactions.log');

// Function to get the latest Ethereum block
async function getLatestEthBlock() {
  try {
    ...
    const logEntry = `[${new Date().toISOString()}] ETHEREUM BLOCK: ${JSON.stringify(blockInfo, null, 2)}\n`;
    ...
    const errorMsg = `Error fetching Ethereum block: ${error.message}`;
...
async function getLatestEthTransactions(count = 5) {
  try {
    console.log(`Fetching latest ${count} Ethereum transactions...`);
    const blockNumber = await provider.getBlockNumber();
    const block = await provider.getBlock(blockNumber);
    ...
    const logEntry = `[${new Date().toISOString()}] ETHEREUM TRANSACTIONS: ${JSON.stringify(transactions, null, 2)}\n`;
    fs.appendFileSync(logFilePath, logEntry);
    
    console.log(`Retrieved ${transactions.length} transactions from block ${blockNumber}`);
    return transactions;
  } catch (error) {
    const errorMsg = `Error fetching Ethereum transactions: ${error.message}`;
    console.error(errorMsg);
    fs.appendFileSync(logFilePath, `[${new Date().toISOString()}] ERROR: ${errorMsg}\n`);
...
async function updateDashboard() {
  try {
    const blockInfo = await getLatestEthBlock();
    const transactions = await getLatestEthTransactions();
    ...
    const dashboardFilePath = path.join(logsDir, 'eth-dashboard.json');
...
async function startAutoRefresh(intervalSeconds = 30) {
  try {
    ...
    setInterval(async () => {
      try {
        await updateDashboard();
        console.log(`Auto-refreshed Ethereum data at ${new Date().toISOString()}`);
      } catch (refreshError) {
...
if (require.main === module) {
  ...
  console.log(" ETHEREUM BLOCKCHAIN INTEGRATION");
  console.log("=".repeat(50));
