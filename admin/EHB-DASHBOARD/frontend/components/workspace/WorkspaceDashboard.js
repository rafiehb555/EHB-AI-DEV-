...
const WorkspaceDashboard = ({ children }) => {
  ...
  useEffect(() => {
    const initActiveModules = () => {
      try {
        ...
        if (userType === 'seller') {
          filteredMo(filteredModules || []).filter(.filter(
  ...
  const handleRemoveModule = (moduleId) => {
    setActive(prev || []).filter( => (prev || []).filter(m => m.id !== moduleId));
  ...
  return (
    <div className={`workspace-dashboard ${layoutClass}`} style={mainContainerStyle}>
      ...
      <div style={contentStyle}>
        ...
        {/* Active modules as dragga(activeModules || []).ma(activeModules || []).map((les || []).map(m<DraggablePanel
            key={module.id}
            id={module.id}
            title={module.title}
            defaultPosition={{ x: 100, y: 100 }}
            defaultSize={{ width: 300, height: 300 }}
            onClose={() =></DraggablePanel>       onClose={() => handleRemoveModule(module.id)}
          >
    ...
    <WorkspaceSettingsPanel
            onClose={() =></WorkspaceSettingsPanel>ttingsPanel
