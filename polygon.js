 ...
 * This script fetches the latest block from the Polygon network
...
const rpcUrl = process.env.POLYGONRPCURL;
const provider = new ethers.JsonRpcProvider(rpcUrl);

// Log file path
const logFilePath = path.join(logsDir, 'polygon-transactions.log');

// Function to get the latest Polygon block
async function getLatestPolygonBlock() {
  try {
    console.log("Connecting to Polygon network...");
    const blockNumber = await provider.getBlockNumber();
    console.log(`Latest Polygon block number: ${blockNumber}`);
    ...
    const logEntry = `[${new Date().toISOString()}] POLYGON BLOCK: ${JSON.stringify(blockInfo, null, 2)}\n`;
    ...
    const errorMsg = `Error fetching Polygon block: ${error.message}`;
...
async function getLatestPolygonTransactions(count = 5) {
  try {
    console.log(`Fetching latest ${count} Polygon transactions...`);
    const blockNumber = await provider.getBlockNumber();
    const block = await provider.getBlock(blockNumber);
    ...
    const logEntry = `[${new Date().toISOString()}] POLYGON TRANSACTIONS: ${JSON.stringify(transactions, null, 2)}\n`;
    fs.appendFileSync(logFilePath, logEntry);
    
    console.log(`Retrieved ${transactions.length} transactions from block ${blockNumber}`);
    return transactions;
  } catch (error) {
    const errorMsg = `Error fetching Polygon transactions: ${error.message}`;
    console.error(errorMsg);
    fs.appendFileSync(logFilePath, `[${new Date().toISOString()}] ERROR: ${errorMsg}\n`);
...
async function updateDashboard() {
  try {
    const blockInfo = await getLatestPolygonBlock();
    const transactions = await getLatestPolygonTransactions();
    ...
    const dashboardFilePath = path.join(logsDir, 'polygon-dashboard.json');
...
async function startAutoRefresh(intervalSeconds = 30) {
  try {
    ...
    setInterval(async () => {
      try {
        await updateDashboard();
        console.log(`Auto-refreshed Polygon data at ${new Date().toISOString()}`);
      } catch (refreshError) {
...
if (require.main === module) {
  ...
  console.log(" POLYGON BLOCKCHAIN INTEGRATION");
  console.log("=".repeat(50));
...
module.exports = {
  getLatestPolygonBlock,
  getLatestPolygonTransactions,
  updateDashboard,
  startAutoRefresh
};